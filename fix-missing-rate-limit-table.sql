-- ============================================
-- FIX: Ensure daily_report_limits table exists
-- ============================================
-- Run this in Supabase SQL Editor if you're getting
-- "relation daily_report_limits does not exist" error
-- ============================================

-- Create table if it doesn't exist
CREATE TABLE IF NOT EXISTS public.daily_report_limits (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id text,  -- Clerk user ID (text format, not UUID)
  ip_address text,
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'daily_report_limits_user_id_report_date_key'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'daily_report_limits_ip_address_report_date_key'
  ) THEN
    ALTER TABLE public.daily_report_limits
...
CREATE INDEX IF NOT EXISTS idx_daily_limits_user_date 
  ON public.daily_report_limits(user_id, report_date);

CREATE INDEX IF NOT EXISTS idx_daily_limits_ip_date 
  ON public.daily_report_limits(ip_address, report_date);

CREATE INDEX IF NOT EXISTS idx_daily_limits_date 
  ON public.daily_report_limits(report_date);
...
-- Add helpful comment
COMMENT ON TABLE public.daily_report_limits IS 'Rate limiting for report generation. user_id is Clerk user ID (text format). Tracks daily report generation per user and IP address.';
...
SELECT 
    ...
    (SELECT COUNT(*) FROM public.daily_report_limits) as current_records;
