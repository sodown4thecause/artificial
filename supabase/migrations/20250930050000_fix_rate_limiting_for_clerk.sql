-- Fix rate limiting to work with Clerk TEXT user IDs instead of UUID

-- Drop RLS policies first (they depend on the user_id column)
...
ALTER TABLE public.daily_report_limits 
  DROP CONSTRAINT IF EXISTS daily_report_limits_user_id_fkey;
...
-- Recreate indexes
DROP INDEX IF EXISTS idx_daily_limits_user_date;
CREATE INDEX idx_daily_limits_user_date ON public.daily_report_limits(user_id, report_date);

-- Update the check_and_increment_daily_limit function to accept TEXT user_id
...
-- Update get_daily_usage_stats to accept TEXT user_id
...
-- Note: RLS policies are managed by the Edge Function with service role key
-- since Clerk authentication happens at the application level, not database level

COMMENT ON TABLE public.daily_report_limits IS 'Rate limiting for report generation. user_id is Clerk user ID (text format).';
